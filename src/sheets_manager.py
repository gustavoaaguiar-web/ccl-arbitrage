...
class SheetsManager:
    ...
    def guardar_historico_diario_cache_batch(self, cache_completo: Dict[str, List[dict]]):
        ...
        """
        ws = self._hojas.get("Historico_Diario_Cache")
        if not ws or not cache_completo:
            return

        filas = [HEADERS["Historico_Diario_Cache"]]
        for symbol, bars in cache_completo.items():
            for b in bars:
                filas.append([
                    symbol, b["t"][:10], round(b["o"], 4), round(b["h"], 4),
                    round(b["l"], 4), round(b["c"], 4), round(b["v"], 2),
        ...
        total_velas = sum(len(bars) for bars in cache_completo.values())
        logger.info(
            f"✅ Historico_Diario_Cache (batch): {len(cache_completo)} símbolos, "
            f"{total_velas} velas totales guardadas en 1 sola escritura"
        )
