...
if df_picos.empty:
    ...
    salida_a_actual = 0.15
    pct_captura = (df_picos["pico_pnl"] >= salida_a_actual).mean() * 100
    st.info(
        f"📌 Con Salida A en +{salida_a_actual:.2f}%, "
        f"el **{pct_captura:.1f}%** de las entradas llega a ese nivel. "
        f"El p75 del pico es +{pico_p75:.3f}% — "
        f"{'subir la Salida A capturaría más ganancia por operación pero cerraría menos operaciones.' if pico_p75 > salida_a_actual else 'la Salida A actual está por encima del p75, podrías bajarla para cerrar más operaciones.'}"
    )
