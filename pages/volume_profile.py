...
def cargar_precios_actual():
    """
    Carga precios actual (CCL) desde Sheets o IOL.
    Retorna dict con precio actual y símbolo.
    """
    try:
        sheets = SheetsManager()
        # Cargar último snapshot de CCL_Historial
...
def verificar_cache_valido(cache_timestamp: datetime) -> bool:
...
def main():
    ...
    if vp_intraday and vp_intraday['es_valido']:
        ...
        # Obtener CCL actual
        precio_actual = cargar_precios_actual()
    ...
    if cache_valido and st.session_state.vp_cache["dos_semanas"]:
        ...
        with st.spinner("Cargando histórico 2 semanas..."):
            try:
                ...
                # Actualizar cache (mismo timestamp)
    ...
    if vp_intraday and vp_intraday['es_valido'] and vp_2sem and vp_2sem['es_valido']:
        ...
        if pocintras_vs_poc2sem > 0:
            st.info(f"✅ El POC intraday está **ARRIBA** del patrón histórico (2 semanas). "
                   f"El CCL se está moviendo hacia precios más altos.")
        elif pocintras_vs_poc2sem < 0:
            st.warning(f"⚠️ El POC intraday está **ABAJO** del patrón histórico (2 semanas). "
                      f"El CCL se está moviendo hacia precios más bajos.")
        else:
            st.success(f"🟢 El POC intraday coincide con el patrón histórico.")
        
        # Proximidad al rango histórico
